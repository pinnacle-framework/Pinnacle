...
.. code-block::
    ...
    command=/bin/bash -c "python3 -m pinnacledb.servers.master $(cat config.json | jq .master.port)"
    ...
    command=/bin/bash -c "python3 -m pinnacledb.servers.jobs $(cat config.json | jq .hash_set.port)"
    process_name=%(program_name)s_%(process_num)s
    numprocs=1
