...
from pinnacle.backends.base.query import (
    Query,
    applies_to,
...
class MongoOutputs(MongoQuery):
    ...
    def _postprocess_result(self, result):
        ...
        """
        pinnacle_outputs = {}
        pinnacle_builds = result.get('_builds', {})
        pinnacle_files = result.get('_files', {})
        pinnacle_blobs = result.get('_blobs', {})
        ...
        for predict_id in predict_ids:
            ...
            pinnacle_outputs[predict_id] = output_result
            pinnacle_builds.update(output_data['_builds'])
            pinnacle_files.update(output_data['_files'])
            pinnacle_blobs.update(output_data['_blobs'])

        result['_outputs'] = pinnacle_outputs
        result['_builds'] = pinnacle_builds
        result['_files'] = pinnacle_files
        result['_blobs'] = pinnacle_blobs
        return result
