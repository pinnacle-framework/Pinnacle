...
from pinnacle.backends.simple.vector_search import SimpleVectorSearchClient
...
def job_worker(job: Job) -> t.Any:
    ...
    os.makedirs(os.path.expanduser('~/.pinnacle/simple'), exist_ok=True)
    with ROUTER.redirect_to(
        os.path.expanduser(f'~/.pinnacle/simple/{job.huuid}.log'), 'w'
    ):
        result = job.run(db)
    # os.environ.clear()
