...
def _legend_panel():
    ...
    [bold blue]_path[/]: This indicates the module path within pinnacle.io.\n
...
class SuperDuperFlatEncode(t.Dict[str, t.Any]):
    ...
    def pinnacle(self, d, inplace=False):
        """Merge two dictionaries.

        :param d: Dict, must have '_base' key
        :param inplace: bool, if True, pinnacle in place
        """
        assert isinstance(d, SuperDuperFlatEncode)
        if '_base' in d:
            assert '_base' in self, "Cannot pinnacle differently encoded data"
