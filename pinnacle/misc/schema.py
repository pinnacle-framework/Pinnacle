...
from pinnacle.misc import typing as pinnacle_typing
...
class Annotation(ABC):
    ...
    def build(annotation: t.Type):
        ...
        if isinstance(annotation, ForwardRef):
            ...
            pinnacle_globals = sys.modules["pinnacle"].__dict__
...
ANNOTATIONS = ArgumentDefaultDict(
    default_factory,  # type: ignore[arg-type]
    {
        ...
        t.Literal: Literal,
        pinnacle_typing.JSON: JSON,
        pinnacle_typing.File: PathAnnotation,
        pinnacle_typing.FileDict: lambda: Dict[Str, PathAnnotation],
        pinnacle_typing.SDict: lambda: Dict[Str, ComponentAnnotation],
        pinnacle_typing.SList: lambda: List[ComponentAnnotation],
        pinnacle_typing.BaseType: BaseAnnotation,
        pinnacle_typing.ComponentType: ComponentAnnotation,
    },
)
...
def gather_mro_globals(cls):
    ...
    for base in cls.__mro__:
        ...
        if hasattr(base, "__module__") and base.__module__ in sys.modules:
            pinnacled.update(sys.modules[base.__module__].__dict__)
    return pinnacled
...
def _safe_get_type_hints(cls: t.Type[t.Any]) -> dict[str, t.Any]:
    ...
    for f in fields(cls):
        hints[f.name] = _safe_resolve_annotation(
            f.type, {**module_globals, **pinnacle_globals, **mro_globals}
        )
        ...
        for field_name, field_type in cls.metadata_fields.items():
            if field_name not in hints:
                hints[field_name] = _safe_resolve_annotation(
                    field_type, {**module_globals, **pinnacle_globals, **mro_globals}
                )
    return hints


def get_schema(cls):
    """Get a schema for a pinnacle class.
