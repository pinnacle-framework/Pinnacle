...
def _map_type_to_pinnacle(source, name, cls, iterable):
    if iterable is None:
...
def process(annotation):
    """Process an annotation with a crude mapping to workable pinnacle types.
    ...
    >>> import typing as t
    >>> from pinnacle import Model, Component
    >>> process(Model)
    (pinnacle.components.model.Model, None)
    >>> process(Model | None)
    (pinnacle.components.model.Model, None)
    >>> process(t.List[str])
    (str, list)
    >>> process(t.Dict[str, Component])
    (pinnacle.components.component.Component, dict)
    ...
    if isinstance(inferred_cls, ForwardRef):
        ...
        inferred_cls = _evaluate_forward_ref(
            inferred_cls, {**module_globals, **pinnacle_globals}
        )
...
def get_schema(cls):
    ...
    for parameter in annotations:
        ...
        if inferred_cls is None:
            ...
            continue
        try:
            schema[parameter] = _map_type_to_pinnacle(
