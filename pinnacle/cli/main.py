...
from pinnacle import CFG, Component, logging, pinnacle
...
def bootstrap(
    ...
    >>> pinnacle bootstrap rag

    Copy template to a directory to update it.

    >>> pinnacle bootstrap rag ./my_template
    """
    from pinnacle import templates as inbuilt
...
def ls():
    ...
    :param values: JSON string of values to apply to the template.
    """
    from pinnacle.templates import ls
...
def execute(data_backend: str = None):
    ...
    from pinnacle.misc.interactive_prompt import _prompt
...
def drop(data: bool = False, force: bool = False, data_backend: str | None = None):
    ...
    """
    data_backend = data_backend or CFG.data_backend
    db = pinnacle(data_backend, initialize_cluster=False)
