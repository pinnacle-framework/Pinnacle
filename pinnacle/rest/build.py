...
from pinnacle.base.leaf import import_item
...
from pinnacle.rest.base import DatalayerDependency, SuperDuperApp
...
def build_rest_app(app: SuperDuperApp):
    ...
    def db_execute(query: t.Dict, db: 'Datalayer' = DatalayerDependency()):
        ...
        if '_path' not in query:
            ...
            query['_path'] = f'pinnacle_{plugin}.query.parse_query'
...
def build_frontend(app: SuperDuperApp, host: str = 'localhost', port: int = 8000):
    ...
    try:
        shutil.rmtree(f"{ROOT}/pinnacletmp")
    except FileNotFoundError:
        pass

    shutil.copytree(f"{ROOT}/out", f"{ROOT}/pinnacletmp")

    DIRECTORY = f"{ROOT}/pinnacletmp"
