...
def up_cluster(notebook_token: t.Optional[str] = None):
    ...
    cmd = f"pinnacleDB_CONFIG={CFG} PYTHONPATH=$(pwd):. {ray_executable} start --head --dashboard-host=0.0.0.0 --disable-usage-stats --block"
    run_tmux_command(['send-keys', '-t', 'ray-head', cmd, 'C-m'])
    time.sleep(10)
    cmd = f"pinnacleDB_CONFIG={CFG} RAY_ENABLE_WINDOWS_OR_OSX_CLUSTER=1 PYTHONPATH=$(pwd):. {ray_executable} start --address=localhost:6379  --block"
