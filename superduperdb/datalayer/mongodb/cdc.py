...
from pinnacledb.queries.mongodb import queries
...
from pinnacledb.vector_search.lancedb import LanceDBClient
...
class MongoDatabaseWatcher(GenericDatabaseWatch, MongoEventMixin):
    ...
    def check_if_pinnacle_change(self, change: t.Dict) -> bool:
        """
        A helper method to check if the cdc change is done
        by pinnacledb queries.
        """
    ...
    def next_cdc(self, stream: CollectionChangeStream) -> None:
        ...
        try:
            ...
            if change is not None:
                ...
                # Who did the change.
                if not self.check_if_pinnacle_change(change):
