...
class Collection(BaseCollection):
    ...
    def apply_model(self, name, r):
        if self.remote:
            return pinnacle_requests.client.apply_model(name, r)
    ...
    def create_imputation(self, name, model, loss, target, metrics=None, filter=None, projection=None,
        ...
        if not self.remote:
            ...
            return [job_id, pinnacle_requests.jobs.process(
                self.database.name,
                self.name,
    ...
    def create_semantic_index(self, name, models, measure, validation_sets=(), metrics=(), loss=None,
        ...
        for model in active_models:
            ...
            if not self.remote:
                ...
                job_ids.append(pinnacle_requests.jobs.process(
                    self.database.name,
                    self.name,
                    '_process_documents_with_model',
                    model,
    ...
    def _find_nearest(self, filter, ids=None):
        if self.remote:
            ...
            return pinnacle_requests.client._find_nearest(self.database.name, self.name, filter,
                                                            ids=ids)
            ...
            for (type_, item) in current:
                if type_ == 'models':
                    ...
                    if not self.remote:
                        ...
                        process_id = pinnacle_requests.jobs.process(
