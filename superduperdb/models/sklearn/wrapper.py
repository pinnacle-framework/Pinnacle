...
from pinnacledb.core.model import Model, TrainingConfiguration
...
class Pipeline(Base):
    def __init__(
        ...
        standard_steps = [
            ...
            if not getattr(s[1], 'pinnacle_postprocess', False)
        ]
        postprocess_steps = [
            i
            for i, s in enumerate(steps)
            if getattr(s[1], 'pinnacle_postprocess', False)
        ]
