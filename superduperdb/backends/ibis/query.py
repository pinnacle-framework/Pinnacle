...
class IbisQuery(Query):
    ...
    def _refactor_insert_data_info(data):
        ...
        for key in ['_leaves', '_blobs', '_files', '_schema']:
            if key in data:
                pinnacle_data[key] = data.pop(key)

        pinnacle_data = base64.b64encode(pickle.dumps(pinnacle_data)).decode()
        data['_info'] = pinnacle_data
        return data

    @staticmethod
    def _recover_insert_data_info(data):
        if '_info' in data:
            pinnacle_data = pickle.loads(base64.b64decode(data.pop('_info')))
            data.update(pinnacle_data)
        return data
