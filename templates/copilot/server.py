...
from pinnacle import pinnacle, logging, CFG
...
IGNORE = '^files|^artifact_store|^.pinnacle_pair_programmer'
...
@app.post("/ask_pinnacle")
def ask_pinnacle(question: Question):
    ...
    db = app.state.db
    m = db.load('Askpinnacle', 'ask_' + PROJECT_NAME)
...
def update_application_if_changed(db):
    ...
    os.makedirs(WATCH_PATH + '/.pinnacle_pair_programmer', exist_ok=True)
    watcher = Watcher(
        WATCH_PATH + '/.pinnacle_pair_programmer',
        db=db,
...
async def startup_event():
    ...
    # 2) Create your pinnacle DB
    db = pinnacle('redis://localhost:6379/0')
    ...
    if PROJECT_NAME not in db.show('Application'):
        ...
        t = Template.read(
            os.path.expanduser('~/pinnacle-io/pinnacle/templates/copilot')
        )
        application = t(project_name=PROJECT_NAME)
        db.apply(application, force=True)

    os.makedirs('.pinnacle_pair_programmer', exist_ok=True)
    if not os.path.exists('.pinnacle_pair_programmer/component.yaml'):
        application = db.load('Application', PROJECT_NAME)
        application.export('.pinnacle_pair_programmer', format='yaml')
