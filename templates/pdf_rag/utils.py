...
def pinnacle_metadatas(metadatas):
    if not metadatas:
        return {}
...
def get_related_documents(db, contexts, match_text=None):
    ...
    for source in contexts:
        ...
        metadata = pinnacle_metadatas([e["metadata"] for e in source_elements])
        page_number = metadata["page_number"]
...
def get_related_pinnacled_documents(db, contexts, match_text=None):
    """
    Convert contexts to a dataframe
    Will pinnacle the same page
    """
    image_folder = os.environ.get("IMAGES_FOLDER", ".cache/images")
    source_ids = [source["_source"] for source in contexts]
    for source_id in source_ids:
...
class Processer(Model):
    def predict(self, contexts, match_text=None, pinnacle=False):
        if pinnacle:
            return get_related_pinnacled_documents(self.db, contexts, match_text)
        else:
