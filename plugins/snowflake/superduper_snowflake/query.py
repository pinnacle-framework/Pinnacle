...
from pinnacle.base.query import Op, Query


def map_pinnacle_query_to_snowpark_query(session, query, primary_id: str = 'id'):
    ...
    for i, part in enumerate(query.parts):
        ...
        if part.name == 'filter':
            ...
            for i, a in enumerate(args):
                if isinstance(a, Query):
                    args[i] = map_pinnacle_query_to_snowpark_query(
                        session, a, primary_id
                    )

            for k, v in kwargs.items():
                if isinstance(v, Query):
                    kwargs[k] = map_pinnacle_query_to_snowpark_query(
