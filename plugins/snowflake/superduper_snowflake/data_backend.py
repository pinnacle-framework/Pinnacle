...
from pinnacle_snowflake.connect import connect, watch_token_file
from pinnacle_snowflake.query import map_pinnacle_query_to_snowpark_query
from pinnacle_snowflake.schema import pinnacle_to_snowflake_schema
...
class SnowflakeDataBackend(BaseDataBackend):
    ...
    def create_tables_and_schemas(self, events: t.List[CreateTable]):
        ...
        for ev in events:
            ...
            column_definitions = ",\n    ".join(
                pinnacle_to_snowflake_schema(schema_obj, ev.primary_id)
            )
            statements.append(
                create_table.format(
    ...
    def create_table_and_schema(self, identifier: str, schema: Schema, primary_id: str):
        ...
        if identifier in self.list_tables():
            return
        native_schema_str = ',\n    '.join(
            pinnacle_to_snowflake_schema(schema, primary_id)
        )
    ...
    def _pinnacle_schemas(self, tables: str):
        """Merge schemas.

        :param tables: The tables to pinnacle.
        """
        fields = {}
        for tab in tables:
    ...
    def missing_outputs(self, query, predict_id):
        ...
        df = map_pinnacle_query_to_snowpark_query(self.session, query, pid)
    ...
    def select(self, query: Query, primary_id: str | None = None) -> t.List[t.Dict]:
        ...
        """
        q = map_pinnacle_query_to_snowpark_query(
            self.session,
            query,
        ...
        logging.info(f"Executing query... DONE ({time.time() - start:.2f}s)")
        pinnacled_schemas = self._pinnacle_schemas(query.tables)
        for k in result.columns:
            if pinnacled_schemas.get(k) == VariantType():
